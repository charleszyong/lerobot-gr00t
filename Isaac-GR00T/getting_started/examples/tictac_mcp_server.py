...
class RobotController:
    ...
    def initialize(self):
        ...
        try:
            ...
            # Initialize the robot - using camera configuration that works with tictac_bot.py
            self.robot = SO100Robot(calibrate=False, enable_camera=True, cam_idx=self.cam_idx)

            # Use the activate/connect pattern from tictac_bot.py
            self.robot.connect()
    ...
    def get_current_image(self):
        ...
        try:
            # Use the same get_current_img method as in tictac_bot.py
            ...
            print_yellow(f"Available observation keys: {self.robot.get_observation().keys() if self.robot else 'No robot initialized'}")
            return None
